...
export class MeasureEditor{
  ...
  render(){this.wrap.innerHTML=''; if(!this.page||!this.measure){this.wrap.innerHTML='<p class="hint">Selecione um compasso.</p>';return;}
    const m=this.measure; m.time_grid=timeGridForMeter(m.meter);
    const layer=document.createElement('div'); layer.className='work-layer'; layer.style.width=m.bbox.w*this.zoom+'px'; layer.style.height=m.bbox.h*this.zoom+'px';
    const img=document.createElement('img'); img.src=this.page.image_src; img.style.position='absolute'; img.style.width=this.page.width*this.zoom+'px'; img.style.left=-m.bbox.x*this.zoom+'px'; img.style.top=-m.bbox.y*this.zoom+'px'; img.draggable=false; layer.appendChild(img);
    ...
    for(const mk of m.markers){const dot=document.createElement('div');dot.className=`marker-dot type-${mk.type}`;dot.style.left=mk.x*this.zoom+'px';dot.style.top=mk.y*this.zoom+'px';dot.title=`${mk.type}: ${mk.value}`;dot.addEventListener('click',e=>{e.stopPropagation();this.editMarker(mk.marker_id);});layer.appendChild(dot); const lab=document.createElement('div');lab.className='marker-label';lab.textContent=mk.value||mk.type;lab.style.left=mk.x*this.zoom+'px';lab.style.top=mk.y*this.zoom+'px';layer.appendChild(lab);}
    let lensEl=null;
    layer.addEventListener('mousemove',e=>{const p=this.pt(e,layer);this.mouse=p;if(!lensEl){lensEl=document.createElement('div');lensEl.className='lens';const li=document.createElement('img');li.src=this.page.image_src;lensEl.appendChild(li);layer.appendChild(lensEl);} lensEl.style.left=(p.x*this.zoom+18)+'px';lensEl.style.top=(p.y*this.zoom+18)+'px'; const li=lensEl.querySelector('img'); li.style.width=this.page.width*this.lensZoom+'px'; li.style.left=-(m.bbox.x+p.x)*this.lensZoom+90+'px'; li.style.top=-(m.bbox.y+p.y)*this.lensZoom+90+'px';});
    layer.addEventListener('mouseleave',()=>{if(lensEl){lensEl.remove();lensEl=null;}});
    layer.addEventListener('click',e=>{const p=this.pt(e,layer);this.protocol.ui_state.cursor_position={x:Math.round(p.x),y:Math.round(p.y)};this.createMarkerAt(p);});
  ...
  createMarkerAt(p){const type=document.getElementById('markerType')?.value||'chord';let value=''; if(type==='chord') value=prompt('Acorde (ex.: D, Bm, A7/G, C/D):','')||''; else if(type==='syllable') value=prompt('Sílaba/letra (ex.: Re-, -nhor, com_o):','')||''; else if(type==='note_head') value=prompt('Duração opcional (ex.: colcheia, semínima):','')||'nota'; else if(type==='navigation') value=prompt('Tipo de navegação (repeat_start, fine, coda...):','')||'navigation'; else if(type==='rest') value='pausa'; else value=prompt('Valor do marcador:','')||type;
    if(!value) return; const beat=nearestBeat(p.x,this.measure.bbox.w,this.measure.time_grid); const mk={marker_id:uid('mk'),type,value,x:Math.round(p.x),y:Math.round(p.y),beat,confidence:'manual',duration:type==='note_head'?value:'',extra:{}}; this.measure.markers.push(mk); if(type==='navigation'){this.protocol.navigation.visual_markers.push({id:uid('nav'),type:value,measure_id:this.measure.measure_id,confidence:'manual'});} pushUndo(this.protocol,{type:'create_marker',measure_id:this.measure.measure_id,marker_id:mk.marker_id}); this.protocol.ui_state.selected_marker_type=type; scanManualSpecialCases(this.measure); updateMeasureConfidence(this.measure); this.onChange(); this.render();}
